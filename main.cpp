...
void ejercicio2_modificar_saturacion() {
  ...
  imshow("Saturacion Aumentada (+50%)", img_resultado);
  waitKey(0);
  destroyAllWindows();
}
...
void ejercicio3_kmeans_manual(int K = 5) {
  ...
  int swatch_size = 50;
  Mat paleta(swatch_size, K * swatch_size, CV_8UC3);
  for (int k = 0; k < K; k++) {
    Rect roi(k * swatch_size, 0, swatch_size, swatch_size);
    paleta(roi).setTo(Scalar(saturate_cast<uchar>(centroides[k].b),
                             saturate_cast<uchar>(centroides[k].g),
                             saturate_cast<uchar>(centroides[k].r)));
  }

  imshow("Original (Small)", img_small);
...
void ejercicio4_gray_world() {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      Vec3b p = img_bgr.at<Vec3b>(i, j);
      img_resultado.at<Vec3b>(i, j) =
          Vec3b(saturate_cast<uchar>(p[0] * scale_b),
                saturate_cast<uchar>(p[1] * scale_g),
                saturate_cast<uchar>(p[2] * scale_r));
    }
  }

  imshow("Original", img_bgr);
  imshow("Gray World (Corregida)", img_resultado);
  waitKey(0);
  destroyAllWindows();
}
...
void ejercicio6_gamma(double gamma = 1.5) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      Vec3b p = img_bgr.at<Vec3b>(i, j);
      img_resultado.at<Vec3b>(i, j) = Vec3b(lut[p[0]], lut[p[1]], lut[p[2]]);
    }
  }

  imshow("Original", img_bgr);
  imshow("Corrección Gamma = " + to_string(gamma), img_resultado);
  waitKey(0);
  destroyAllWindows();
}
...
void ejercicio7_vignette(double k = 0.4) {
  ...
  double cx = cols / 2.0;
  double cy = rows / 2.0;
  double d_max = sqrt(pow(cx, 2) + pow(cy, 2));
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      ...
      double f = 1.0 / (1.0 - k * pow(d_norm, 2));
  ...
  imshow("Original", img_bgr);
  imshow("Vignette Corregido k=" + to_string(k), img_resultado);
  waitKey(0);
  destroyAllWindows();
}
...
int main() {
  ...
  while (opcion != 0) {
    cout << "\n====================================" << endl;
    cout << "      TALLER OPENCV: COLOR      " << endl;
    cout << "====================================" << endl;
    cout << "1. BGR -> HSV (Manual)" << endl;
    cout << "2. Modificar Saturación (+50%)" << endl;
    cout << "3. K-Means Manual (Cuantización)" << endl;
    cout << "4. Algoritmo Gray World" << endl;
    cout << "5. Corrección Gamma (1.5)" << endl;
    cout << "6. Corrección de Viñeteo" << endl;
    cout << "0. Salir" << endl;
    cout << "------------------------------------" << endl;
